...
int Login(Student students[], std::string id, std::string password) {
	if (students[toHash(id)].id == id) {
		if (students[toHash(id)].password == password) {
			int now = toHash(id); //now는 현재 접속자의 index
			return toHash(id);
		}
		else return -1;
	}
...
void Match(Student students[],  int now) {
	...
	for (int no = 0; no < 500; no++) {
		...
		students[no].score = students[no].score - 5 * match_mbti(students[no].mbti, students[now].mbti);
	}
...
void open_csv_and_make_professor(Professor professors[]) {
